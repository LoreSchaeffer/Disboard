...
export const stopButtonRSC: RemoteCommand = {
    op: 'StopButton',
    handler: (ws, msg) => {
        ...
        if (boardType === 'music') {
            sendCommandAck(ws, 'StopButton', false, 'Stopping music buttons is not supported. Please use the "Stop" command instead.');
            return;
        }

        if (!isBoardOpen(boardType)) {
            sendCommandAck(ws, 'StopButton', false, `${boardType} board is not open`);
            return;
        }
        ...
        if (!activeProfile) {
            sendCommandAck(ws, 'StopButton', false, 'Active profile not found');
            return;
        }
        ...
        if (!button) {
            sendCommandAck(ws, 'StopButton', false, 'Button not found');
            return;
        }

        const board = getBoardWin(boardType);
        board.webContents.send('player:on_stop_button', button.id);

        sendCommandAck(ws, 'StopButton', true);
    }
}